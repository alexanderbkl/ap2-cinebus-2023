...
def show(g: BusesGraph) -> None:
    ...
    nodes_gdf = gpd.GeoDataFrame([attr for node, attr in g.nodes(data=True)], geometry=gpd.points_from_xy(
        [attr['x'] for node, attr in g.nodes(data=True)], [attr['y'] for node, attr in g.nodes(data=True)]))
    edges_gdf = gpd.GeoDataFrame([attr for node1, node2, attr in g.edges(data=True)], geometry=[LineString([Point(
        g.nodes[node1]['x'], g.nodes[node1]['y']), Point(g.nodes[node2]['x'], g.nodes[node2]['y'])]) for node1, node2 in g.edges()])
